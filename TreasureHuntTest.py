...
def extract_code_cells(notebook_path):
    ...
    code_cells = [
        cell["source"] for cell in nb_contents.cells if cell.cell_type == "code"
    ]
    return code_cells
...
class TestTreasureHuntGame(unittest.TestCase):
    def test_winning_path(self):
        ...
        self.assertGreaterEqual(
            exec_globals["player_score"], 150
        )  # Access variables through exec_globals
        self.assertEqual(exec_globals["player_health"], 100)
    ...
    def test_losing_path(self):
        ...
        with patch("builtins.input", side_effect=choices):
            exec("play()", exec_globals)
        self.assertEqual(exec_globals["player_score"], 0)  # Player should have 0 points
        self.assertEqual(
            exec_globals["player_health"], 0
        )  # Player health should reach 0
...
def main():
    ...
    exec_globals = execute_code(
        code_cells
    )  # Execute the code cells and store the globals
